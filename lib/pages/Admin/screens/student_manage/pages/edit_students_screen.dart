...
class EditStudentScreen extends StatefulWidget {
  ...
  bool get isFemale => student.isFemale;
  String get dob => student.dob;
  String get group => student.group;
  String get memorization => student.memorization;
  String get username => student.username;
  String get password => student.password;
  String? get parentName => student.parentName;
  String? get parentPhone => student.parentPhone;
  String? get studentPhone => student.studentPhone;
  List<bool> get docs => student.docs;
...
class _EditStudentScreenState extends State<EditStudentScreen>
  ...
  static const List<String> _memOptions = [
    'لا يوجد حفظ سابق',
    'جزء عمّ',
    'من 1 إلى 5 أجزاء',
    'من 5 إلى 10 أجزاء',
  ...
  void initState() {
    ...
    _memorization = _memOptions.contains(widget.memorization)
        ? widget.memorization
        : _memOptions.first;

    _guardianNameCtrl = TextEditingController(text: widget.parentName ?? '');
    _guardianPhoneCtrl = TextEditingController(text: widget.parentPhone ?? '');
    _studentPhoneCtrl = TextEditingController(text: widget.studentPhone ?? '');
  ...
  void _handleSave() {
    ...
    final v2 = _parentKey.currentState?.validate() ?? true;
    final v3 = _accountKey.currentState?.validate() ?? true;
    if (!v1 || !v2 || !v3) {
      if (!v1)
  ...
  void _handleReset() {
    ...
    _parentKey.currentState?.reset();
    _accountKey.currentState?.reset();
    setState(() {
      _nameCtrl.text = widget.studentName;
      _dob = _parseDob(widget.dob);
      ...
      _memorization = _memOptions.contains(widget.memorization)
          ? widget.memorization
          : _memOptions.first;
      _guardianNameCtrl.text = widget.parentName ?? '';
      _guardianPhoneCtrl.text = widget.parentPhone ?? '';
      _studentPhoneCtrl.text = widget.studentPhone ?? '';
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                Navigator.pop(context); // يغلق EditStudentScreen
              },
    ...
    return AppBar(
      ...
      bottom: TabBar(
        ...
        tabs: const [
          ...
          Tab(icon: Icon(Icons.family_restroom, size: 18), text: 'ولي الأمر'),
                          ...
                          if (wide)
                            Row(
                              ...
                              children: [
                                ...
                                const SizedBox(width: 16),
                                Expanded(child: _buildMemDropdown()),
                              ],
                            )
                          else ...[
                            _buildGroupDropdown(),
  ...
  Widget _buildGroupDropdown() => DropdownButtonFormField<String>(
    ...
    validator: (v) => v == null ? 'يرجى اختيار الفوج' : null,
  );

  Widget _buildMemDropdown() => DropdownButtonFormField<String>(
    initialValue: _memorization,
    decoration: _decor(
      label: 'كمية الحفظ',
      hint: 'اختر كمية الحفظ',
    ...
    validator: (v) => v == null ? 'يرجى اختيار كمية الحفظ' : null,
  );

  // ════════════════════════════════════════
  // تبويب 2 — معلومات ولي الأمر
  // ════════════════════════════════════════
                      ...
                      Container(
                        ...
                        child: Row(
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                'الطالب بالغ ($_age سنة) — معلومات ولي الأمر غير مطلوبة.',
                                style: TextStyle(
                    ...
                    _sectionCard(
                      ...
                      child: wide
                          ? Row(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Expanded(child: _buildGuardianNameField()),
                                const SizedBox(width: 16),
                                Expanded(child: _buildGuardianPhoneField()),
                              ],
                            )
                          : Column(
                              children: [
  ...
  Widget _buildGuardianNameField() => TextFormField(
    ...
    validator: (v) {
      if (!_isAdult && (v == null || v.trim().isEmpty))
        return 'هذا الحقل مطلوب';
      return null;
    },
  );

  Widget _buildGuardianPhoneField() => TextFormField(
